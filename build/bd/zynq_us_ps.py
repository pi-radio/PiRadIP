...
@BDIP.register
class Zynq_US_PS(BDIP):
    bd_automation = "xilinx.com:bd_rule:zynq_ultra_ps_e"
    vlnv = "xilinx.com:ip:zynq_ultra_ps_e:3.4"

    clkin_map = {
        "M_AXI_HPM0_FPD": "maxihpm0_fpd_aclk",
        "M_AXI_HPM1_FPD": "maxihpm1_fpd_aclk",
    }
    ...
    def __init__(self, parent, name, **kwargs):
        ...
        self.pl_clk = [ self.pins["pl_clk0"] ]
        self.pl_resetn = [ self.pins["pl_resetn0"] ]
    ...
    def setup_aximm(self):
        self.parent.make_active()
        
        clock_map = {
            "M_AXI_HPM0_FPD": "pl_clk0",
            "M_AXI_HPM1_FPD": "pl_clk0",
        }


        clocks = sorted(set(clock_map.values()))

        self.ps_reset = list()

        self.aximm_clocks = list()
        
        for i, n in enumerate(clocks):
            clk = self.pins[n]
            
            print(f"Creating PS Reset for {n}...")
            ps_reset = BDPSReset(self.parent, f"{n}_rst_block", None)
            
            clk.assoc_resetn = ps_reset.pins["peripheral_aresetn"]
            
            self.ps_reset.append(ps_reset)
            
            net = clk.create_net(f"aximm{i}_net")
            self.aximm_clocks.append(net)
        ...
        for p in filter(is_aximm, self.pins.values()):
            clk = self.pins[clock_map[p.name]]

            clk.connect(self.pins[self.clkin_map[p.name]])
            
            self.aximm_overrides[p.name] = { 'clk': clk, 'rst': clk.assoc_resetn }
...
"""        
    default_props = [
        ...
        ("CONFIG.PSU__PROTECTION__FPD_SEGMENTS", "SA:0xFD1A0000; SIZE:1280; UNIT:KB; RegionTZ:Secure; WrAllowed:Read/Write; subsystemId:PMU Firmware |  SA:0xFD000000; SIZE:64; UNIT:KB; RegionTZ:Secure; WrAllowed:Read/Write;subsystemId:PMU Firmware |  SA:0xFD010000; SIZE:64; UNIT:KB; RegionTZ:Secure; WrAllowed:Read/Write; subsystemId:PMU Firmware |  SA:0xFD020000; SIZE:64; UNIT:KB; RegionTZ:Secure; WrAllowed:Read/Write; subsystemId:PMUFirmware |  SA:0xFD030000; SIZE:64; UNIT:KB; RegionTZ:Secure; WrAllowed:Read/Write; subsystemId:PMU Firmware |  SA:0xFD040000; SIZE:64; UNIT:KB; RegionTZ:Secure; WrAllowed:Read/Write; subsystemId:PMU Firmware|  SA:0xFD050000; SIZE:64; UNIT:KB; RegionTZ:Secure; WrAllowed:Read/Write; subsystemId:PMU Firmware |  SA:0xFD610000; SIZE:512; UNIT:KB; RegionTZ:Secure; WrAllowed:Read/Write; subsystemId:PMU Firmware|  SA:0xFD5D0000; SIZE:64; UNIT:KB; RegionTZ:Secure; WrAllowed:Read/Write; subsystemId:PMU Firmware | SA:0xFD1A0000 ; SIZE:1280; UNIT:KB; RegionTZ:Secure ; WrAllowed:Read/Write; subsystemId:Secure Subsystem"),
...
; SIZE:64; UNIT:KB; RegionTZ:Secure ; WrAllowed:Read/Write; subsystemId:Secure Subsystem|SA:0xFF180000 ; SIZE:768; UNIT:KB; RegionTZ:Secure ; WrAllowed:Read/Write; subsystemId:Secure Subsystem|SA:0xFF9A0000 ; SIZE:64; UNIT:KB; RegionTZ:Secure ; WrAllowed:Read/Write; subsystemId:Secure Subsystem"),
